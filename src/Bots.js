...
function Bots() {
  ...
  useEffect(() => {
    const fetchBots = async () => {
      ...
      try {
        const response = await axios.get(`https://gestion-rpa-backend.vercel.app/api/bots?userId=${userId}`);
  ...
  const handleExecute = async (botId, idAAServer, idBotAA, nombreBot, servidor) => {
    ...
    try {
      const response = await axios.post(`https://gestion-rpa-backend.vercel.app/api/execute-bot`, {
        idAAServer,
        idBotAA,
        nombreBot,
        servidor,
