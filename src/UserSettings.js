...
const UserSettings = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserInfo = async () => {
      try {
        const response = await fetch(`https://fronted-gestion-rpa.vercel.app/api/user-info?userId=${userId}`);
        if (response.ok) {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      const response = await fetch(`https://fronted-gestion-rpa.vercel.app/api/change-password`, {
        method: 'POST',
        headers: {
